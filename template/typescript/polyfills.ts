...
function getValuePolyfill(key: string): any {
  return mockValues[key];
}

function setValuePolyfill(key: string, value: any): void {
  mockValues[key] = value;
  console.log(`[Mock] setValue: ${key} =`, value);
}
...
if (typeof globalThis !== "undefined") {
  ...
  if (!globalThis.getValue) {
    (globalThis as any).getValue = getValuePolyfill;
  }
  if (!globalThis.setValue) {
    (globalThis as any).setValue = setValuePolyfill;
  }
  if (!globalThis.consoleLog) {
    (globalThis as any).consoleLog = console.log;
  }
  if (!globalThis.consoleError) {
    (globalThis as any).consoleError = console.error;
  }
  if (!globalThis.jsonParse) {
    (globalThis as any).jsonParse = JSON.parse;
  }
}
